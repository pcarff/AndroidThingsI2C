...
public class EzoPhCircuitDriver implements AutoCloseable {

    private static final String TAG = EzoPhCircuitDriver.class.getSimpleName();
     ...
     * @see #register()
     */
    public EzoPhCircuitDriver(String bus) throws IOException {
    ...
    public void sleep() {
        if (mPhProbe !=null) {
            try {
                mPhProbe.setMode(PhProbe.MODE_STANDBY);
            } catch (IOException e) {
    ...
    public void awake() {
        if (mPhProbe !=null) {
            try {
                mPhProbe.setMode(PhProbe.MODE_ACTIVE);
            } catch (IOException e) {
        ...
        return new UserSensor.Builder()
                ...
                .setDriver(new UserSensorDriver() {
                    ...
                    public void setEnabled(boolean enabled) throws IOException {
                        if (enabled) {
                            phprobe.setMode(PhProbe.MODE_ACTIVE);
                        } else {
                            phprobe.setMode(PhProbe.MODE_STANDBY);
                        }
                    }
                })
                .build();
